...
local Groups = require("tokyomoonight.groups")
...
function M.extras()
  local Extra = require("tokyomoonight.extra")
...
function M.plugins()
  ...
  for _, name in ipairs(names) do
    ...
    t[#t + 1] = {
      link(repo, group.url),
      link(("`%s`"):format(name), "lua/tokyomoonight/groups/" .. name .. ".lua"),
    }
  end
  return M.table(t)
end
...
function M.update()
  local config = Docs.extract("lua/tokyomoonight/config.lua", "\n(--@class tokyomoonight%.Config.-\n})")
